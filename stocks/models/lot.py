...
class Lot(models.Model):
    numero_lot = models.CharField(max_length=100, unique=True, verbose_name="N° de lot")
    ...
    quantite_initiale = models.DecimalField(
        max_digits=18, decimal_places=6, verbose_name="Quantité initiale",
    )
    ...
    def __str__(self):
        return f"Lot {self.numero_lot} — {self.article.code}"


class NumeroSerie(models.Model):
    numero = models.CharField(max_length=100, unique=True, verbose_name="N° de série")
    article = models.ForeignKey(
        "Article",
        on_delete=models.CASCADE,
